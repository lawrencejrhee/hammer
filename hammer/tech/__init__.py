...
class HammerTechnology:
    ...
    def override_tech_libraries(self) -> None:
        ...
        if self.get_setting("vlsi.technology.manually_override_pdk_collateral"):
            ...
            if len(self.get_setting("vlsi.technology.override_libraries") or []) > 0:
        ...
        used_overrides = {}
        for lib in self.config.libraries + (self.config.drc_decks or []) + (self.config.lvs_decks or []):
    ...
    def prepend_dir_path(self, path: str, lib: Optional[Library] = None) -> str:
        ...
        "installs": [{
            "root": {
                "id": "pdkroot",
                "path": "/nfs/ecad/techname100/stdcells/"
            }
        }],
        "libraries": [{
            "name": "lib1",
        ...
        and refers to a file relative to that identifier's path
            pdkroot/dac/dac.lib -> /nfs/ecad/techname100/stdcells/dac/dac.lib
