...
from hammer.vlsi import CLIDriver
# Removed unused import: from hammer.vlsi.cli_driver import import_task_to_dag
#import pdb
#pdb.set_trace()
...
def create_hammer_dag_gcd():
    # Helper function to get param value (checks conf first, then params, then default)
    ...
    def start(**context):
        """Start task - routes to clean (which then flows to build_decider) or exit"""
        has_steps = (get_param(context, 'build', True) or 
            get_param(context, 'sim_rtl', True) or
            get_param(context, 'syn', True) or
            get_param(context, 'par', True))
    ...
    @task
    def clean(**context):
        """Clean the build directory (only if clean param is True, otherwise pass through)"""
        if get_param(context, 'clean', False):
            print("Clean parameter is True, cleaning build directory")
            flow = AIRFlow()
            if os.path.exists(flow.OBJ_DIR):
                subprocess.run(f"rm -rf {flow.OBJ_DIR} hammer-vlsi-*.log", shell=True, check=True)
        else:
            print("Clean parameter is False, skipping clean (passing through to build)")
    ...
    def sim_or_syn_decide(**context):
        ...
        if get_param(context, 'sim_rtl', True):
            return 'sim_rtl'
        elif (get_param(context, 'syn', True) or 
            get_param(context, 'par', True)):
            return 'syn_decider'
        return 'exit_'
    ...
    def build_decider(**context):
        ...
        if get_param(context, 'build', True):
            return 'build'
        elif (get_param(context, 'sim_rtl', True) or
            get_param(context, 'syn', True) or
            get_param(context, 'par', True)):
    ...
    def syn_decider(**context):
        ...
        if get_param(context, 'syn', True):
            return 'syn'
        elif (get_param(context, 'par', True)):
            return "par_decider"
        else:
            return "exit_"
    ...
    # Set up dependencies
    # Flow: start -> clean (conditional) -> build_decider -> build -> sim_or_syn -> ...
    start >> [clean, exit_]
    clean >> build_decide
...
def create_hammer_dag_rocket():
    ...
    def start(**context):
        ...
        has_steps = (get_param(context, 'build', True) or 
            get_param(context, 'sim_rtl', True) or
            get_param(context, 'sram_generator', True) or
            get_param(context, 'syn', True) or
            get_param(context, 'par', True))
        if has_steps or get_param(context, 'clean', False):
            return "clean"
    ...
    def sim_or_syn_decide(**context):
        ...
        if get_param(context, 'sim_rtl', True):
            return 'sim_rtl'
        elif (get_param(context, 'sram_generator', True) or 
            get_param(context, 'syn', True) or
            get_param(context, 'par', True)):
            return 'sram_decider'
        return 'exit_'
    ...
    def sram_generator(**context):
        ...
        if get_param(context, 'sram_generator', True):
    ...
    def syn(**context):
        """Execute synthesis task"""
        print("Starting syn task")
        if (get_param(context, 'sram_generator', True) or
         get_param(context, 'syn', True)):
            print("Synthesis parameter is True, executing syn")
            flow = AIRFlow_rocket()
            flow.syn()
        else:
    ...
    def exit_():
        """Exit task"""
        print("Exiting")
        # Removed sys.exit(0) - it kills the Airflow worker process
