"""Example Airflow DAGs kept from early SledgeHammer development.

Nothing imports these; per-design DAGs are generated by
hammer.vlsi.hammer_build_systems instead. They are parked here rather
than in hammer_vlsi.py so that importing the CLI does not require
airflow: a plain hammer user has no reason to install it.
"""
